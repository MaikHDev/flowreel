...
export const ParticipantRoles = pgEnum("ParticipantRoles", ["member", "admin"]);
...
export const messages = createTable(
  ...
  (t) => [index().on(t.senderId), index().on(t.conversationId)],
);
...
export const messageReadRelations = relations(
  messagesReadStatus,
  ({ one }) => ({
    user: one(users, {
      fields: [messagesReadStatus.userId],
      references: [users.id],
    }),
    message: one(messages, {
      fields: [messagesReadStatus.messageId],
      references: [messages.id],
    }),
  }),
);
